# Claude Code statusline — 由 ClientKit statusline-builder 產生
...
# token-in — always/dash
$v = $d.context_window.current_usage.input_tokens
if ($null -eq $v) {
  $vt = '--'
} else {
  $vt = (Format-Tokens $v)
}
$disp = [char]0x69 + [char]0x6E + [char]0x3A + ' ' + $vt
$Segs += "$e[0m$e[38;5;80m" + $disp

# token-out — always/dash
$v = $d.context_window.current_usage.output_tokens
if ($null -eq $v) {
  $vt = '--'
} else {
  $vt = (Format-Tokens $v)
}
$disp = [char]0x6F + [char]0x75 + [char]0x74 + [char]0x3A + ' ' + $vt
$Segs += "$e[0m$e[38;5;81m" + $disp
...
$disp = [char]0x63 + [char]0x61 + [char]0x63 + [char]0x68 + [char]0x65 + [char]0x3A + ' ' + $vt
$Segs += "$e[0m$e[38;5;214m" + $disp
...
if ($null -ne $v -and $Now -lt $v) {
  $disp = [char]0x72 + [char]0x35 + [char]0x68 + [char]0x3A + ' ' + (Format-Reset5h $v $Now)
  $Segs += "$e[0m$e[38;5;99m" + $disp
}

# reset-7d — conditional/hide
$v = $d.rate_limits.seven_day.resets_at
if ($null -ne $v -and $Now -lt $v) {
  $disp = [char]0x72 + [char]0x37 + [char]0x64 + [char]0x3A + ' ' + (Format-Reset7d $v $Now)
  $Segs += "$e[0m$e[38;5;99m" + $disp
}
